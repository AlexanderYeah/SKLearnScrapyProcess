...
def get_one_group_img(url):
    ...
    # 开启多线程 获取到每一张图片的url 地址，那么就进行写入本地操作
    open_multi_thread(res_imgs_list);
...
def loop1(i,img_url):
    # 切换到指定目录
    os.chdir(target_path);
    print ("start loop %s at %s"%(i,ctime()));
    ...
    print ("end loop %s at %s"%(i,ctime()));

# 开启线程数 根据传入的图片链接数目
def open_multi_thread(img_list):
    threads = [];
    nloops = range(len(img_list));
    for i in  nloops:
        # 创建线程
        t  = threading.Thread(target=loop1,args=(i,img_list[i]));
        # 将线程装进 数组里面
        threads.append(t);
